...
function plotEnsemble(ensemble_cv, ensemble_time, n_points, t_max )
    ...
    n_points = minimum([size(ensemble_cv[i])[1] for i=1:n_traj])

    t_traj, cv_traj = unifyTimeEnsembleTimePoints(ensemble_cv, ensemble_time, n_points, t_max)

    n_states = size(cv_traj)[2]

    mean = zeros(n_points, n_states)
    ...
    for i = 1:n_traj
        plot!(plt,t_traj, cv_traj[:,1,i], linecolor="gray", legend=false)
    end
    plot!(t_traj, mean[:,1], linecolor="red")
    display(plt)
end


function unifyTimeEnsembleTimePoints(ensemble_cv, ensemble_time, n_points, t_max)
    # Returns all trajectories sampled at equidistant points
    
    n_traj = length(ensemble_cv)
    num_states = size(ensemble_cv[1])[2]
    
    time_points = [i/n_points*t_max for i=1:n_points]

    # Ordering of indices is swapped compared to ensemble cv to benefit from column major order
    ...
    for i = 1:n_traj
        ...
        for j = 1:num_states
            ...
            while old_index < size(ensemble_cv[i])[1]

                if  ensemble_time[i][old_index] >= time_points[new_index]
                    while  ensemble_time[i][old_index] >= time_points[new_index]
            ...
            # Fix Trailing zeros in new array if there are more new points than existing points
